...
def _compare_realdata() -> Path:
    ...
    cells.append(nbf.v4.new_code_cell(
        ...
        "\n"
        "def venn2_fixed(set_a, set_b, label_a: str, label_b: str, ax, *,\n"
        "                color_a: str = '#1f77b4', color_b: str = '#2ca02c',\n"
        "                alpha: float = 0.55) -> None:\n"
        "    \"\"\"Two-set Venn with fixed-size overlapping circles.\n"
        "\n"
        "    matplotlib_venn collapses to a single circle on perfect overlap and\n"
        "    its 1.1.x venn2_unweighted hits an internal `normalize_to vs custom\n"
        "    layout` ValueError, so we draw the regions ourselves — three counts\n"
        "    in fixed positions, set labels below each circle (no collision).\n"
        "    \"\"\"\n"
        "    a_only = len(set_a - set_b)\n"
        "    b_only = len(set_b - set_a)\n"
        "    both   = len(set_a & set_b)\n"
        "    ax.add_patch(Circle((-0.55, 0), 1.0, facecolor=color_a, edgecolor='none', alpha=alpha, zorder=1))\n"
        "    ax.add_patch(Circle(( 0.55, 0), 1.0, facecolor=color_b, edgecolor='none', alpha=alpha, zorder=1))\n"
        "    ax.text(-1.10, 0, f'{a_only:,}', ha='center', va='center', fontsize=11, zorder=2)\n"
        "    ax.text( 1.10, 0, f'{b_only:,}', ha='center', va='center', fontsize=11, zorder=2)\n"
        "    ax.text( 0.00, 0, f'{both:,}',    ha='center', va='center', fontsize=11, zorder=2)\n"
        "    ax.text(-0.55, -1.25, label_a, ha='center', va='top', fontsize=10, color=color_a, weight='bold')\n"
        "    ax.text( 0.55, -1.25, label_b, ha='center', va='top', fontsize=10, color=color_b, weight='bold')\n"
        "    ax.set_xlim(-2.0, 2.0); ax.set_ylim(-1.75, 1.3)\n"
    ...
    cells.append(nbf.v4.new_code_cell(
        ...
        "xmax = max(r['R_min'] for r in summary_rows)\n"
        "ax.set_xlim(0, xmax * 1.32)  # 32% right margin reserves space for value labels\n"
        "pad = xmax * 0.012\n"
        "for i, r in enumerate(summary_rows):\n"
        "    ax.text(r['py_min'] + pad, i - h/2, f\"{r['py_min']:.2f} min\", va='center', fontsize=9, color='#1f77b4')\n"
        "    ax.text(r['R_min']  + pad, i + h/2, f\"{r['R_min']:.2f} min  ({r['speedup']}x)\", va='center', fontsize=9, color='#d62728')\n"
        "ax.set_yticks(y); ax.set_yticklabels(labels)\n"
        "ax.set_xlabel('wall-clock (minutes)')\n"
        "ax.set_title('End-to-end wall-clock — pycopykat vs R copykat')\n"
        "# Legend below the axes, horizontal — keeps it clear of the value labels.\n"
    ...
    for idx, (cancer, sample) in enumerate(REALDATA_SAMPLES):
        ...
        cells.append(nbf.v4.new_code_cell(
            ...
            "venn2_fixed(py_aneu, r_aneu, 'pycopykat', 'R copykat', ax=axes[0])\n"
            "axes[0].set_title('Cells called \"aneuploid\"', fontsize=11, pad=10)\n"
            "conf = pd.crosstab(adata.obs['py_class'], adata.obs['r_class']).reindex(\n"
